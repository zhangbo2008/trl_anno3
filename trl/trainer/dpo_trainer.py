...
class DPOTrainer(Trainer):
        ...
        for k in batch:
            if k.startswith("rejected") and isinstance(batch[k], torch.Tensor):
                ...
                concatenated_batch[concatenated_key] = torch.cat(
                    ...
                    dim=0,
                ).to(device=device) #=========把chosen, reject拼起来.
    ...
    def dpo_loss(
        ...
        logits = pi_logratios - ref_logratios #======让这个最大就行了.
...
#=======核心函数, 找出来 概率.
    ...
    def get_batch_logps(
        ...
        if not is_encoder_decoder:
            labels = labels[:, 1:].clone() # 大部分都是decoder模型,所以需要偏移一个单位, 第一个token不是生成的. 其他token是生成的.
            logits = logits[:, :-1, :] #同时logits生成的概率,最后一个也不需要,只有[:-1]才跟labels对应上.
        loss_mask = labels != label_pad_token_id

        # dummy token; we'll ignore the losses on these tokens later
        labels[labels == label_pad_token_id] = 0

        per_token_logps = torch.gather(logits.log_softmax(-1), dim=2, index=labels.unsqueeze(2)).squeeze(2) # 概率拿出来即可, 根据label抽取即可.

        if average_log_prob: #=是否输出均值.
            return (per_token_logps * loss_mask).sum(-1) / loss_mask.sum(-1)
        else:
            return (per_token_logps * loss_mask).sum(-1) #输出sum
#====更快的forwad.
    def concatenated_forward(
        ...
        """
        concatenated_batch = self.concatenated_inputs( #===拼接正反input
            batch,
            is_encoder_decoder=self.is_encoder_decoder,
